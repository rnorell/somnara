...
-- Keep existing Auth users ready for device pairing and account sync.
insert into public.profiles(id, email, name)
select
  u.id,
  left(coalesce(u.email, ''), 320),
  left(coalesce(u.raw_user_meta_data->>'full_name', u.raw_user_meta_data->>'name', split_part(coalesce(u.email, ''), '@', 1)), 100)
